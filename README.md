...
1. これがオブジェクト指向プログラミングだ<br>
英語でゆうとオブジェクトオリエンテッドプログラミング（省略：OOP）<br>
結論：OOPとは現実世界の物に見立ててプログラムする方法<br>
どのようにアプリケーションを作るかという方法の1つ<br>
人間にとってわかりやすくロジックを分割する方法の1つ<br>
...
2. classを定義してオブジェクトを作成する方法<br>
class.tsファイルを参照<br>

3. JSにコンパイルされたclassはこうなっている<br>
よく理解出来てない<br>

4. クラスにメソッドを追加する方法<br>
class.tsファイルを参照<br>
ES6とTypeScriptの書き方は一緒<br>

5. クラスを型として使う方法<br>
class.tsファイルを参照<br>

6. public修飾子とprivate修飾子を使用して、アクセスを制限する方法<br>
簡単にゆうと、クラスの中で定義されているデータとゆうものが外部からアクセスができるか出来ないの制御をするもの<br>
class.tsファイルを参照<br>
パブリックは、クラス内、クラス外どこでも読み書きが出来る<br>
プライベートは、クラス内で読み書きは出来るが、クラス外では読み書き出来ない<br>

7. 初期化の処理を省略する方法<br>
class.tsファイルを参照<br>

8. readonly修飾子を使って、書き換え出来ないようにする方法<br>
class.tsファイルを参照<br>
フィールドやパラメーターに付ける事が出来る<br>
クラス内、クラス外でも「読むだけ、書けない」の役割<br>

9. extendsを使用して、他のクラスの機能を継承する方法<br>
class.tsファイルを参照<br>

10. protected修飾子を使用して、継承先までアクセス出来る範囲を広げる方法<br>
class.tsファイルを参照<br>
クラス内、継承先で使えるが、クラス外では使えない<br>

11. ゲーターとセッターはこう作る<br>
何かを取得したデータを取得した時に何かの関数を実行したい時にゲッターを使うらしい<br>
何かの値を代入したい、何かの値を変更したい時にこうゆう処理ををして変更する時に何かの処理をしたい時にセッターを使うらしい<br>
そもそもTypeScriptのものではなく、ES6には存在するものらしい<br>
class.tsファイルを参照<br>

12. staticを使用して、インスタンスを作らずにクラスを使う方法<br>
class.tsファイルを参照<br>

13. Abstractクラスを使用して、継承にのみ使えるクラスを作成する方法<br>
class.tsファイルを参照<br>
Abstractnの意味は抽象的なって意味らしい<br>
注意点としてインスタンスを生成出来ないところがあるらしい<br>
継承のみに使われる事が主にらしい<br>

14. private（プライベート）をconstructor（コンストラクター）に付けて、シングルトンパターンを実装する方法<br>
class.tsファイルを参照<br>
シングルトンパターンとは、クラスからインスタンスを1つしか作れないとゆう意味らしい（デザインパターンでもあるらしい）<br>
インスタンスを作らなくても使えるものはスタティックメソッドとゆうのがあるらしい<br>

## これがInterfaceだ！
インターフェースとは、シンプルにゆうとオブジェクトの型になるらしい<br>

1. interfaceはこう使う！

2. メソッドをオブジェクトの型に指定する方法

3. implements（インプリメントズ）を使用して、クラスに対してinterfaceの条件を適応させる方法

4. これがTypeScriptの構造的部分型だ！

5. readonly（リードオンリー）修飾子をinterfaceに使って読むだけのプロパティを作る方法

6. extends（エクステンドズ）を使ってinterfaceを継承する方法

7. interfaceで関数の型を表現する方法

8. ？を使って、あってもなくても良いオプショナルプロパティとオプショナルパラメーターを使用する方法