...
class.tsファイルを参照
パブリックは、クラス内、クラス外どこでも読み書きが出来る
プライベートは、クラス内で読み書きは出来るが、クラス外では読み書き出来ない
...
class.tsファイルを参照
フィールドやパラメーターに付ける事が出来る
クラス内、クラス外でも「読むだけ、書けない」の役割
...
class.tsファイルを参照
クラス内、継承先で使えるが、クラス外では使えない

11. ゲーターとセッターはこう作る
何かを取得したデータを取得した時に何かの関数を実行したい時にゲッターを使うらしい
何かの値を代入したい、何かの値を変更したい時にこうゆう処理ををして変更する時に何かの処理をしたい時にセッターを使うらしい
そもそもTypeScriptのものではなく、ES6には存在するものらしい
class.tsファイルを参照
...
class.tsファイルを参照
Abstractnの意味は抽象的なって意味らしい
注意点としてインスタンスを生成出来ないところがあるらしい
継承のみに使われる事が主にらしい

14. privateをconstructorに付けて、シングルトンパターンを実装する方法
class.tsファイルを参照
シングルトンパターンとは、クラスからインスタンスを1つしか作れないとゆう意味らしい（デザインパターンでもあるらしい）
インスタンスを作らなくても使えるものはスタティックメソッドとゆうのがあるらしい